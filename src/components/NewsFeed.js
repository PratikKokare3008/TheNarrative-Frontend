...
// UPDATED CONSTANTS - New working API URL
const API_URL = process.env.REACT_APP_API_URL || 'https://narrative-ml-cloudrun-53060812465.asia-south2.run.app';
...
const NewsFeed = ({
  ...
  const newsAPI = useMemo(() => ({
    fetchArticles: async (pageParam = 1) => {
      ...
      try {
        console.log(`Fetching from ${API_URL}/analyze?${params}`);
        
        // Use the new working API URL for bias analysis
        const response = await axios.post(`${API_URL}/analyze`, {
          text: searchQuery || 'General news analysis request',
        ...
        // Transform the bias analysis response into news article format
        const transformedData = {
          articles: [{
            id: `analysis-${Date.now()}`,
            title: 'News Bias Analysis',
            description: response.data.explanation || 'Bias analysis completed',
            content: response.data.original_text || searchQuery || 'Sample news content',
            summary: response.data.explanation || 'AI-powered bias analysis',
            articleBias: response.data.bias_type || 'center',
            biasConfidence: response.data.confidence || 0.85,
            biasScore: response.data.bias_score ? Math.round(response.data.bias_score * 100) : 50,
            source: {
              name: 'AI Analysis',
              url: API_URL
            },
            ...
            urlToImage: null,
            category: 'analysis',
            biasReasoning: response.data.explanation || 'Automated bias detection analysis'
          }],
          pagination: {
        ...
        if (window.gtag) {
          window.gtag('event', 'news_fetch_success', {
            event_category: 'API',
            page: pageParam,
            articles_count: transformedData.articles.length,
    ...
    fetchStories: async (pageParam = 1) => {
      // For stories, we'll return a mock response since the new API doesn't have stories endpoint
      return {
        storyGroups: [],
        pagination: {
          currentPage: pageParam,
    ...
    fetchStats: async () => {
      // Return mock stats for the new API
      return {
        overview: {
          totalArticles: 1,
          recentArticles: 1,
    ...
    reportError: async (error) => {
      try {
        await axios.post(`${API_URL}/health`, {
          error: {
            message: error.message,
            stack: error.stack,
            timestamp: Date.now(),
  ...
  // ENHANCED INTERSECTION OBSERVER FOR LAZY LOADING AND ANALYTICS
  useEffect(() => {
